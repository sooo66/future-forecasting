...
class SubstackImporter(BaseImporter):
    ...
    def __init__(self, config: ImportConfig) -> None:
        ...
        self.request_pause_sec = max(0.0, float(config.request_pause_sec))
    ...
    def _fetch_new_posts(
        ...
        while True:
            ...
            for post in posts:
                ...
                if cutoff and post_dt and post_dt <= cutoff:
                    return all_posts
                if not self.config.include_paid_posts and not self._is_free_post(post):
                    continue
        ...
        feed_posts = self._fetch_feed_posts(
            base_url=base_url,
            author=author,
            cutoff=cutoff,
            end_date=end_date,
        ...
        if feed_posts:
            seen = {self._post_key(p) for p in all_posts}
            ...
            logger.info(f"Merged feed fallback posts: +{len(all_posts) - before_count}")
        return all_posts

    @staticmethod
    def _post_key(post: Dict[str, Any]) -> str:
        for key in ("canonical_url", "id", "slug"):
            value = post.get(key)
            if value:
                return str(value)
    ...
    def _fetch_archive_page(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"Archive request failed: {url} ({exc})")
            return []
    ...
    def _fetch_post_detail(self, base_url: str, slug: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Post detail request failed: {url} ({exc})")
            return {}
    ...
    def _fetch_feed_posts(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"Feed request failed: {feed_url} ({exc})")
            return []

        try:
            root = ET.fromstring(xml_text)
        except ET.ParseError:
            logger.warning(f"Feed parse failed: {feed_url}")
            return []
        ...
        for item in root.findall(".//item"):
            ...
            if pub_date_raw:
                try:
                    ...
                    post_date_iso = parsed.astimezone(timezone.utc).isoformat()
                except (TypeError, ValueError):
                    post_date_iso = None

            post_dt = self._parse_datetime(post_date_iso)
            if end_date and post_dt and post_dt > end_date:
                continue
            if cutoff and post_dt and post_dt <= cutoff:
                continue
            ...
            for child in list(item):
                ...
                if tag_name.endswith("encoded"):
                    body_html = (child.text or "").strip()
                    if body_html:
                        break
            slug = self._extract_slug_from_url(link)
            content = self._clean_post_content(body_html, None, description)
            posts.append(
                {
                    "id": (item.findtext("guid") or "").strip() or None,
                    "title": title,
...
def load_import_config(config_path: str | Path) -> ImportConfig:
    ...
    return ImportConfig(
        ...
        request_pause_sec=float(request.get("pause_sec", 0.2)),
        include_paid_posts=bool(ingestion.get("include_paid_posts", True)),
